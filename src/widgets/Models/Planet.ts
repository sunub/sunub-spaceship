...
export class Planet extends BaseModel {
	...
	async debug() {
		...
		f.addBinding(this.position, "x", { min: -100, max: 100, step: 0.1 }).on(
			"change",
			(ev) => {
		...
		f.addBinding(this.position, "y", { min: -100, max: 100, step: 0.1 }).on(
			"change",
			(ev) => {
		...
		f.addBinding(this.position, "z", { min: -100, max: 100, step: 0.1 }).on(
			"change",
			(ev) => {
