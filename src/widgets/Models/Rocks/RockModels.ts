...
export class Rocks extends BaseModel {
	...
	private generateInstancePosition(
		...
		for (let attempt = 0; attempt < maxAttempts; attempt++) {
			...
			const position = new THREE.Vector3(
				x,
				this.positionOptions.positionY,
				z,
			)
		...
		this.rockInstances.forEach((instance) => {
			...
			this.meshesToProcess.forEach((template) => {
				...
				if (indices && indices.length > 0) {
					...
					colliderDesc =
						RAPIER.ColliderDesc.convexHull(transformedVertices)
				}
	...
	update(_deltaTime: number): void {
		...
		this.rockInstances.forEach((instance) => {
			if (
				instance.rigidBody &&
				instance.rigidBody.bodyType() === RAPIER.RigidBodyType.Dynamic
			) {
	...
	private updatePosition(options: PositionOptions): void {
		...
		// modelGroup은 원점(0,0,0)에 고정하고 개별 인스턴스들의 좌표를 직접 업데이트합니다.
		// 이렇게 함으로써 물리 엔진의 월드 좌표계와 렌더링 엔진의 월드 좌표계를 일치시킵니다.
