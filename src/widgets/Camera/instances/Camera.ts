...
export class Camera
{
	 ...
	 * @param targetObject 추적할 Object3D (예: 스페이스십) 또는 null (추적 중단)
	...
	setupPan()
	{
		...
		this.pan.disable = () =>
		{
			...
			// Update cursor
			this._context.renderer.domElement.classList.remove(
				"has-cursor-grab",
			)
		}
		...
		this.pan.down = (_x, _y) =>
		{
			...
			// Update cursor
			this._context.renderer.domElement.classList.add(
				"has-cursor-grabbing",
			)
