...
export class FlightController {
  ...
  public handleMovement(rigidBody: RAPIER.RigidBody, deltaTime: number): void {
    ...
    const currentRotation = new THREE.Quaternion().copy(rigidBody.rotation() as THREE.Quaternion);
    const currentLinvel = new THREE.Vector3().copy(rigidBody.linvel() as THREE.Vector3);
    const currentAngvel = new THREE.Vector3().copy(rigidBody.angvel() as THREE.Vector3);

    // 3. Calculate Target Velocities based on the *smoothed* input
    const { targetLinvel, targetAngvel } = this.calculateTargetVelocities(currentRotation);
    ...
    const torque = this.calculateCorrectiveTorque(targetAngvel, currentAngvel);
  ...
  private updateSmoothedInputs(): void {
    ...
    this.smoothedThrustInput += (this.thrustInput - this.smoothedThrustInput) * this.inputSmoothness;
    this.smoothedRollInput += (this.rollInput - this.smoothedRollInput) * this.inputSmoothness;

    // Prevent tiny values from causing drift
    if (Math.abs(this.smoothedThrustInput) < 0.001) this.smoothedThrustInput = 0;
   ...
   */
  private calculateTargetVelocities(currentRotation: THREE.Quaternion): { targetLinvel: THREE.Vector3, targetAngvel: THREE.Vector3 } {
    // Target linear velocity (using smoothed input)
    const localForward = new THREE.Vector3(1, 0, 0);
    const worldForward = localForward.clone().applyQuaternion(currentRotation);
    const targetLinvel = worldForward.multiplyScalar(this.smoothedThrustInput * this.maxSpeed);

    // Target angular velocity (using smoothed input)
    const targetAngvel = new THREE.Vector3(0, -this.smoothedRollInput * THREE.MathUtils.degToRad(this.turnSpeed), 0);
   ...
   */
  private calculateCorrectiveForce(targetVelocity: THREE.Vector3, currentVelocity: THREE.Vector3): THREE.Vector3 {
    const velocityError = new THREE.Vector3().subVectors(targetVelocity, currentVelocity);
   ...
   */
  private calculateCorrectiveTorque(targetVelocity: THREE.Vector3, currentVelocity: THREE.Vector3): THREE.Vector3 {
    const velocityError = new THREE.Vector3().subVectors(targetVelocity, currentVelocity);
  ...
  public setMaxSpeed(speed: number): void { this.maxSpeed = speed; }
  public getMaxSpeed(): number { return this.maxSpeed; }
  public setTurnSpeed(speed: number): void { this.turnSpeed = speed; }
  public getTurnSpeed(): number { return this.turnSpeed; }
  public setForceFactor(factor: number): void { this.forceFactor = factor; }
  public getForceFactor(): number { return this.forceFactor; }
  public setTorqueFactor(factor: number): void { this.torqueFactor = factor; }
  public getTorqueFactor(): number { return this.torqueFactor; }
  public setInputSmoothness(smoothness: number): void { this.inputSmoothness = smoothness; }
  public getInputSmoothness(): number { return this.inputSmoothness; }
}
