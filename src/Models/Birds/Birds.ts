...
export class Birds extends ResourceModel {
    protected override mesh: InstancedMesh | null = null
    ...
    // Override loadModel to implement procedural logic
    protected async loadModel(addToScene: boolean = true): Promise<void> {
        this.modelGroup = new Group() // Re-initialize or use parent's? Parent inits to null.
        this.modelGroup.name = "BirdsGroup"

        this.initBuffers()
        this.initComputeLogic()
        this.initMesh()

        if (this.modelGroup && addToScene) {
    ...
    public update(_deltaTime: number): void {
        ...
        this.rendering.renderer.compute(this.computeVelocity)
        this.rendering.renderer.compute(this.computePosition)
    }

    public dispose(): void {
        if (this.mesh && this.modelGroup) {
