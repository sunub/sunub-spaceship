...
export class CollisionSensor {
    ...
    public initDebugMesh(): Mesh {
        if (!this.config) {
            throw new Error(
                "CollisionSensor must be setup with config before initializing debug mesh.",
            )
        }
    ...
    public update(
        ...
        const upOffset = upVector.clone().multiplyScalar(this.config.offset.up)
        ...
        this.isObstacle = !!(
            hit && hit.time_of_impact < this.config.detectionRange
        )
