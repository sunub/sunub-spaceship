    ...
    parse(buffer) {
        ...
        const /* default error routine.  change this to change error handling */
            ...
            rgbe_error = (rgbe_error_code, msg) => {
                switch (rgbe_error_code) {
                    case rgbe_read_error:
                        throw new Error(
                            `THREE.HDRLoader: Read Error: ${msg || ""}`,
                        )
                    case rgbe_write_error:
                        throw new Error(
                            `THREE.HDRLoader: Write Error: ${msg || ""}`,
                        )
                    case rgbe_format_error:
                        throw new Error(
                            `THREE.HDRLoader: Bad File Format: ${msg || ""}`,
                        )
                    case rgbe_memory_error:
                        throw new Error(
                            `THREE.HDRLoader: Memory Error: ${msg || ""}`,
                        )
                    default:
                        throw new Error(
                            `THREE.HDRLoader: Memory Error: ${msg || ""}`,
            ...
            fgets = (buffer, lineLimit, consume) => {
                ...
                i = chunk.indexOf(NEWLINE)
                while (i < 0 && len < lineLimit && p < buffer.byteLength) {
                    s += chunk
                    len += chunk.length
                    p += chunkSize
                    chunk += String.fromCharCode.apply(
            ...
            RGBE_ReadHeader = (buffer) => {
                ...
                line = fgets(buffer)
                if (buffer.pos >= buffer.byteLength || !line) {
        ...
        switch (this.type) {
            ...
            default:
                throw new Error(
                    `THREE.HDRLoader: Unsupported type: ${this.type}`,
                )
        }
