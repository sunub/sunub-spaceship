...
export class SpaceShipInputHandler {
    constructor(
        @inject(GAME_CONTEXT.MANAGER.InputManager) private readonly inputManager: InputManager,
        @inject(GAME_CONTEXT.CONTROLLER.JoyStick) private readonly joyStick: JoyStick,
    ) {}
    ...
    public getControlState(deltaTime: number, shipPosition: Vector3): SpaceShipControlState {
        // 조이스틱 업데이트
        ...
        const isKeyboardActive = Math.abs(thrustInput) > 0.1 || Math.abs(rollInput) > 0.1;
        ...
        const isActionActive =
            ...
            (pointerVector !== null && (Math.abs(pointerVector.x) > 0 || Math.abs(pointerVector.y) > 0));
