...
export class SpaceShipDebugger {
    ...
    public initialize(
        ...
        const rollHelper = new ArrowHelper(new Vector3(1, 0, 0), new Vector3(0, 0, 0), 1.5, 0xff0000);
        const yawHelper = new ArrowHelper(new Vector3(0, 1, 0), new Vector3(0, 0, 0), 1.5, 0x00ff00);
        const pitchHelper = new ArrowHelper(new Vector3(0, 0, 1), new Vector3(0, 0, 0), 1.5, 0x0000ff);

        this._debugGroup.add(rollHelper);
        this._debugGroup.add(yawHelper);
        this._debugGroup.add(pitchHelper);

        this._debugNormalArrow = new ArrowHelper(new Vector3(0, 1, 0), new Vector3(0, 0, 0), 1.0, 0xffff00);
    ...
    private _setupTweakPane(
        ...
        const f = pane.addFolder({ title: "SpaceShip Debug Controls", expanded: true });

        const PARAMS = { debugMode: this._debugMode };
        f.addBinding(PARAMS, "debugMode", { label: "Show Visuals" }).on("change", (ev) => {
    ...
    public get debugGroup(): Group { return this._debugGroup; }
    public get debugMode(): boolean { return this._debugMode; }
    public get debugNormalArrow(): ArrowHelper | null { return this._debugNormalArrow; }
}
